...
test('select all in input', () => {
    ...
    expect(getUISelection(element)).toEqual({selectionStart: 0, selectionEnd: 11})
    expect(element).toHaveProperty('selectionStart', 0)
    expect(element).toHaveProperty('selectionEnd', 11)
})

test('select all in textarea', () => {
    const { element } = setup<HTMLTextAreaElement>(`<textarea>foo\nbar\nbaz</textarea>`)
...
test('select all outside of editable', () => {
    ...
    expect(selection).toHaveProperty('focusOffset', element.ownerDocument.body.childNodes.length)
})
